...
let mapper =
  object (self)
    ...
    method! expression e =
      ...
      | Pexp_constant _ | Pexp_ident _ -> e
      | Pexp_variant (l, eopt) ->
      ...
      | Pexp_array es -> { e with pexp_desc = Pexp_array (List.map ~f:self#expression es) }
      ...
      | Pexp_constraint (ee, t) -> { e with pexp_desc = Pexp_constraint (self#expression ee, t) }
      ...
      | Pexp_assert e -> { e with pexp_desc = Pexp_assert (self#expression e) }
      | Pexp_lazy e1 ->
          let e1 = self#expression e1 in
      ...
      | Pexp_newtype (name, ee) -> { e with pexp_desc = Pexp_newtype (name, self#expression ee) }
